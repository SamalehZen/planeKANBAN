...
class WorkspaceFileAssetProxyUploadEndpoint(BaseAPIView):
    """
    Proxy upload endpoint that uploads files through the server to R2/S3.
    This bypasses CORS issues when direct browser-to-R2 uploads are blocked.
    """
    ...
    def entity_asset_save(self, asset_id, entity_type, asset, request):
        if entity_type == FileAsset.EntityTypeContext.WORKSPACE_LOGO:
            ...
            workspace.save()
            invalidate_cache_directly(path="/api/workspaces/", url_params=False, user=False, request=request)
            invalidate_cache_directly(path="/api/users/me/workspaces/", url_params=False, user=True, request=request)
            invalidate_cache_directly(path="/api/instances/", url_params=False, user=False, request=request)
        elif entity_type == FileAsset.EntityTypeContext.PROJECT_COVER:
            project = Project.objects.filter(id=asset.project_id).first()
            if project is None:
    ...
    def post(self, request, slug):
        ...
        asset = FileAsset.objects.create(
            ...
            created_by=request.user,
            entity_type=entity_type,
            **self.get_entity_id_field(entity_type=entity_type, entity_id=entity_identifier if entity_identifier else None),
        )
